...
- **`main.tex`:** $\psiin,\psiout$ (conjecture note after `\cref{def:probDefTransducer}`, `main.tex:139`).
- **Definition:** the $\text{LTL}_f$ language of the traces a transducer
  produces — $\psiin$ for $\Tin$, $\psiout$ for $\Tout$; it is what the
  monolithic conjecture (`main.tex:139`) feeds in as the assumption (for $\Tin$)
...
- **`main.tex`:** — (no symbol; a Spot data structure, `\cite duret.25.ciaa`. The
  `\na` at `main.tex:350` gestures at it — *"This likely requires adjusting the
...
- **`main.tex`:** — (no symbol; a **code-only** data structure on the
  *Representation* axis, the nondeterministic sibling of *MTDFA*. The `\na` at
  `main.tex:350` gestures at MTDFA for Method 3 but commits to no NFA form.)
...
- **Definition:** which strategy a transducer represents, selecting its
  ...
  block (`main.tex:136`, $\lambda_C:Q_C\times2^{\mathcal I}\to2^{\Ofree}$). Unlike
...
- **Definition:** subset-determinize an NFA into a DFA. A DFA state is a set
  ...
  reachability invariant `main.tex:253` (a single transducer-state pair per
...
- **C++:** `ProductState` (a struct — `unsigned goal` + `std::vector<unsigned>
  ...
  *Output-agreement automaton* of each transducer lifted by
  ...
  invariant at `main.tex:253`. $\cons$ is applied as the region
...
- **`main.tex`:** $[\psi]$ (`main.tex:355`, §`otf`) — the representative of $\psi$
...
- **Definition:** what makes two progressed formulae the **same** state.
  ...
  **mints a terminal**, so `ForwardProgression::decode` already *returns* $[\psi]$.
  ...
  `main.tex:355` literally claims — flagged for `/theory-review`.
...
- **C++:** per *Representation*, both → `std::optional<Controller>` (`nullopt` =
  ...
  aborting once the initial state is determined, which is what `main.tex:351`'s
  ...
  **not canonical yet** (the PRD's `otf_solve_fused` is explicitly tentative and
  ...
  returns. Both discharge the same `main.tex:315` `\na` by different routes — see
  *Open theory questions*.
...
- **Definition:** the variables $\Xdep$ is dependent **on**. Each direction's
  ...
  - **outputs** — once $\Oknown=\Xdep$, $\Ydep=\mathcal{I}\cup\Ofree$, which is
    $\Sigma_0$ for `Role::t_out` (`main.tex:133`). $\Sout$ moves **last** and so
...
`docs/prd/otf-mtdfa-product.md` — `OtfMtdfaProduct`). `main.tex:350`'s `\na`
...
### Representation
- **`main.tex`:** — (no symbol; the `\na` at `main.tex:350` gestures at MTDFA for
...
- **Definition:** the internal linchpin oracle — given $\varphi,\Tin,\Tout$ and a
  ...
  **directly on agreement**, not on the monolithic conjecture (`main.tex:139`);
...
- **On-the-fly game solving** — Method 3 builds the product on the fly but still
  ...
  `\na` continues (`main.tex:350`): *"This likely requires adjusting the definitions
...
- **Governed-variable projection** (`main.tex:315` `\na`) — *"Because the resulting
  ...
  out** (`main.tex:317–318`), so the claim is currently unbacked in the live text.
...
- **Input dependencies need a different notion** — `\cref{def:outdep}` is
  ...
  **alone**, ignoring $\mathcal{O}$ — a strictly stronger condition and a
  ...
  `latex/main.tex:624–629` block gropes toward ("a potential set of dependent
