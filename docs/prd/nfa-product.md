...
the **reachability-invariant note** at `main.tex:253`
...
- [x] theory-review   — *closed 2026-07-17* — code ↔ math faithfulness vs main.tex.
  **Verdict: Method 1 is theory-faithful; no `code-bug`.** All three open questions
  ...
  so `nfa_to_dfa`'s ∅-skip is sound; (Q2) the `main.tex:253` reachability invariant
...
in $\varphi$ and polynomial in the transducers (`main.tex:253`).
...
2. **Product $P$ (`alg:nfa_product:cons`).** $P=N_c\times\Tin\times\Tout$ with
   ...
   **iff** $\cons(q_{in},q_{out},v)$, else $\emptyset$ (`main.tex:210`–`\cref{alg:nfa_product}`).
...
3. **Determinize $D=\algname{NfaToDfa}(P)$ (`alg:nfa_product:determinize`).**
   ...
   **reachability invariant** (`main.tex:253`) every reachable $R$ has the form
...
- **Phase 1 — `nfa_to_dfa` + `build_product_nondet`.** The generic subset
  ...
  pair (the `main.tex:253` reachability invariant, as an assertion/test). *May stub:*
...
- **Phase 1 structural (no MONA).** `nfa_to_dfa` on hand-built NFAs: ∅-skip, the
  ...
  $(q_{in},q_{out})$ pair (the `main.tex:253` reachability invariant).
...
- **`alg:nfa_product` complexity proof "To be determined"** and the
  **reachability invariant** (`main.tex:253`) — the invariant is asserted here as a
  ...
  **[theory-review 2026-07-17 — RESOLVED, faithful.]** The distinction is correct
  ...
  black box (main.tex:280) states no rule for the empty subset, and both sources of an
  ...
  $A$. Drafted `\cl` note to place after the reachability note at ~main.tex:253
