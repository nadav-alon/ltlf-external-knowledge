...
(main.tex:139–141); `\cref{def:consistency}` (Case-A totality) for why a $\Tin$ run is
...
- [x] glossary        — "Produced-trace language" ($\psiin/\psiout$) + "Faithfulness
      ...
      deferred by decision (open conjecture, main.tex:139, not implemented)
...
- [x] theory-review   — code ↔ math faithful (2026-07-05, `/theory-review`); no
      code-bug. main.tex:141 "known soundness boundary" is a stale misdiagnosis
...
prove the monolithic conjecture (main.tex:139) — it only removes a false
...
- **Produced-trace language of a transducer** — the $\text{LTL}_f$ language
  ...
  (main.tex:139 names it informally, "the $\text{LTL}_f$ languages of the traces
...
- **The "known soundness boundary" in main.tex:141 is false as stated.** The `\cl`
...
- **The monolithic conjecture (main.tex:139) is still open.** This PRD removes its
...
- `docs/prd/ltlfsynt-oracle.md` "Excluded class"/"Open theory questions" and the
  `docs/BACKLOG.md` seed corrected; `main.tex:141` flagged for `/theory-review`.
...
- **Glossary gate left unticked.** Both flagged gaps ("produced-trace language of
  ...
  to decide.
- **`main.tex:141` (the `\cl` note misdiagnosing the "known soundness boundary")
